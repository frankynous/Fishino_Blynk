...
//
// Updated 22-MAY-2015 :
//		cleanup and fixed an uninitialized variable
//		Adapted to Fishino wireless system
//
...
#define FISHINO_MODULE "FishinoWebServer"
#define DEBUG 0
...
void *malloc_check(size_t size)
{
	...
	if (!r)
	{
		Serial << F("TWS:No space for malloc: ");
...
static const FishinoWebServer::MimeType text_html_content_type = 4;

FishinoWebServer::FishinoWebServer(PathHandler handlers[], const char** headers, const int port)
	: handlers_(handlers), server_(port), path_(NULL), request_type_(UNKNOWN_REQUEST), client_()
{
	if (headers)
	{
		int size = 0;
		for (int i = 0; headers[i]; i++)
			size++;
		headers_ = (HeaderValue*)FISHINO_MALLOC(sizeof(HeaderValue) * (size + 1));
		if (headers_)
		{
...
void FishinoWebServer::begin()
{
	server_.begin();
}

// Process headers.
boolean FishinoWebServer::process_headers()
{
	if (headers_)
	{
...
boolean FishinoWebServer::is_requested_header(const char** header)
{
	if (!headers_)
		return false;
	for (int i = 0; headers_[i].header; i++)
...
boolean FishinoWebServer::assign_header_value(const char* header, char* value)
{
	if (!headers_)
		return false;
	...
	for (int i = 0; headers_[i].header; i++)
	{
		...
		{
			headers_[i].value = (char*)FISHINO_MALLOC(strlen(value) + 1);
			if (!headers_[i].value)
				return false;
...
void FishinoWebServer::send_error_code(Client& client, int code)
{
#if DEBUG
...
void FishinoWebServer::send_content_type(MimeType mime_type)
{
...
void FishinoWebServer::send_content_type(const char* content_type)
{
	client_ << content_type_msg;
	client_.println(content_type);
}

const char* FishinoWebServer::get_path()
{
	return path_;
}

const FishinoWebServer::HttpRequestType FishinoWebServer::get_type()
{
	return request_type_;
}

const char* FishinoWebServer::get_header_value(const char* name)
{
	if (!headers_)
		return NULL;
...
char* FishinoWebServer::decode_url_encoded(const char* s)
{
	if (!s)
		return NULL;

	char* r = (char*)FISHINO_MALLOC(strlen(s) + 1);
	if (!r)
		return NULL;
...
char* FishinoWebServer::get_file_from_path(const char* path)
{
...
FishinoWebServer::MimeType FishinoWebServer::get_mime_type_from_filename(const char* filename)
{
...
void FishinoWebServer::send_file(SdFile& file)
{
	size_t size;
...
boolean FishinoWebServer::read_next_char(Client& client, uint8_t* ch)
{
	if (!client.available())
		return false;
...
// read a chunk of characters from input stream up to a given maximum
// return number of characters read
size_t FishinoWebServer::read_next_chars(Client &client, uint8_t *buf, size_t maxSize)
{
	if(!client.available())
		return 0;
	return client.read(buf, maxSize);
}


boolean FishinoWebServer::get_line(char* buffer, int size)
{
	int i = 0;
	char ch;
...
char* FishinoWebServer::get_field(const char* buffer, int which)
{
	char* field = NULL;
	int i = 0;
	int field_no = 0;
	...
	if (field_no == which && i < size)
	{
		...
		field = (char*) FISHINO_MALLOC(j - i + 1);
		if (!field)
			return NULL;
...
// The PUT handler.
namespace FishinoWebPutHandler
{
	HandlerFn put_handler_fn = NULL;

	// Fills in `buffer' by reading up to `num_bytes'.
	// Returns the number of characters read.
	int read_chars(FishinoWebServer& web_server, Client& client, uint8_t* buffer, int size)
	{
		return web_server.read_next_chars(client, buffer, size);
	}

	boolean put_handler(FishinoWebServer& web_server)
	{
		...
		FishinoClient &client = web_server.get_client();
