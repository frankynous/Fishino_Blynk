...
//
// Ported 15 May 2015 to Fishino by <fishino@fishino.com>

#ifndef __FISHINO_WEB_SERVER_H__
#define __FISHINO_WEB_SERVER_H__
...
namespace FishinoWebPutHandler
{
	...
	typedef void (*HandlerFn)(FishinoWebServer& web_server, PutAction action, char* buffer, int size);
...
class FishinoWebServer : public Print
{
	public:
		...
		typedef boolean(*WebHandlerFn)(FishinoWebServer& web_server);
		...
		inline void end_headers()
		{
			client_.println();
		}
		static inline void end_headers(Client& client)
		{
			client.println();
		}
		...
		virtual boolean should_stop_processing()
		{
		...
		virtual size_t read_next_chars(Client &client, uint8_t *buf, size_t maxSize);
