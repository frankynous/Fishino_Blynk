...
#include <Wire.h>
#include "RA8875_I2C.h"

#define RA8875_DATA_ADDR	0x06
#define RA8875_CMD_ADDR		0x07

// write data to RA8875
void  RA8875::writeData(uint8_t data)
{
	Wire.beginTransmission(RA8875_DATA_ADDR);
	Wire.write(data);
...
{
	Wire.beginTransmission(RA8875_CMD_ADDR);
	Wire.write(d);
	Wire.endTransmission();
}
...
{
	if(stat)
		Wire.requestFrom(RA8875_CMD_ADDR, 1);
	else
		Wire.requestFrom(RA8875_DATA_ADDR, 1);
...
// constructor - gets i2c address (last 2 bits) and reset pin
RA8875::RA8875(const uint8_t addr, const uint8_t rst)
{
	_i2cAddr = addr;
	_rst = rst;
}

// Initialize library and Wire
// Parameter:
...
void RA8875::initialize(uint8_t initIndex)
{
	...
	if (_size == Adafruit_480x272 || _size == Adafruit_800x480)
		GPIOX(true);//only for adafruit stuff
	...
	if (_textCursorStyle == BLINK)
		showCursor(false,BLINK); //set default text cursor type and turn off
...
*/
/**************************************************************************/
void RA8875::setActiveWindow(uint16_t x1, uint16_t x2, uint16_t y1, uint16_t y2)
{
	if (x2 >= _width)
		x2 = _width - 1;
	if (y2 >= _height)
...
void RA8875::getActiveWindow(uint16_t &x1, uint16_t &x2, uint16_t &y1, uint16_t &y2)
{
...
void RA8875::getXY(uint16_t &x, uint16_t &y)
{
...
void RA8875::drawFlashImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t address)
{
	// flash config
	uint8_t sroc =
		(1 << 7)	|		// flash1
	...
	setActiveWindow(x, x + w - 1, y, y + h - 1);
...
{
	if (r < 1)
		rectHelper(x,y,w,h,color,filled);//??? reason?
...
// block move
void RA8875::blockMove(uint16_t sx, uint16_t sy, uint8_t sl, uint16_t dx, uint16_t dy, uint8_t dl, uint16_t w, uint16_t h)
{
...
void RA8875::blockVScroll(uint16_t sx, uint16_t sy, uint16_t w, uint16_t h, int16_t dy, uint8_t lay, uint16_t fillColor)
{
	// move the area
	if(dy > 0)
	{
