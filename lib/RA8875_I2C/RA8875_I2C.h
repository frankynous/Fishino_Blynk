...
#ifndef _RA8875_I2C_H_
#define _RA8875_I2C_H_
...
{
	private:
		// reset pin arduino connection
		uint8_t		_rst;
		
		// i2c board address (last 2 bits, 0..3)
		uint8_t		_i2cAddr;
	...
	public:
//------------- Instance -------------------------
		RA8875(const uint8_t addr, const uint8_t rst = 0xff);
...
//--------------area & color -------------------------
		void		setActiveWindow(uint16_t x1, uint16_t x2, uint16_t y1, uint16_t y2);
		void		getActiveWindow(uint16_t &x1, uint16_t &x2, uint16_t &y1, uint16_t &y2);
...
//		void 		drawFlashImage(int16_t x,int16_t y,int16_t w,int16_t h,uint8_t picnum);
		void		drawFlashImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t address);
